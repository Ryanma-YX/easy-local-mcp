...
Fresh installations do not implicitly register with the public Relay. The Control Center pre-fills the public Relay as a suggestion, but the Agent refuses to start until a Relay has been explicitly saved. Existing installations with a valid `worker.json`, or environments with `LOCALMCP_WORKER_URL`, remain compatible and are treated as already configured.

Worker re-registration is performed by the Agent through authenticated control IPC. The browser supplies a validated Worker origin and may supply an optional registration token over the authenticated loopback UI. That token is never returned by the API, never written to `localmcp.json`, never added to audit events, and is stored only in the restrictive `registration-token.pending` state file until registration succeeds and the new local Worker credentials have been persisted; the Agent then deletes it. Agent/MCP credentials remain in the protected Worker state. If `LOCALMCP_WORKER_URL` or `LOCALMCP_REGISTRATION_TOKEN` is set, the UI treats that corresponding value as environment-managed and refuses to override it. Switching Worker replaces local registration state but does not claim to revoke a previous Worker's remote device registration; revoke or rotate that old registration separately when required.
