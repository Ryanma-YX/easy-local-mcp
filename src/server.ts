...
type N=keyof typeof schemas;
const ro=new Set<N>(['workspace_info','list_workspaces','list_directory','workspace_tree','stat_path','find_files','search_files','read_file','read_file_lines','list_skills','read_skill','read_process','list_processes','list_mcp_servers','list_mcp_tools']);
const openWorld=new Set<N>(['run_command','start_process','read_process','write_process','stop_process','list_processes','list_mcp_tools','call_mcp_tool']);
...
export async function createServer(config:Config,mcp:McpLoader,skills:Skill[],processes=new ProcessManager(),getRuntime=()=>({config,mcp,skills})){
  const server=new Server({name:'easy-local-mcp',version:'0.3.10'},{capabilities:{tools:{}}});
  ...
  server.setRequestHandler(CallToolRequestSchema,async req=>{
    ...
    try{
      ...
      if(!decision.allowed){
        if(privileged)await auditSecurity('privileged_tool_denied',{tool:name,workspace:a.workspace,reason:decision.reason,durationMs:Date.now()-started});
        throw new Error(decision.reason||'Tool is not authorized');
      }
      if(privileged)await auditSecurity('privileged_tool_attempt',{tool:name,workspace:a.workspace});

      const workspaces=new Map(Object.entries(config.workspaces).map(([workspaceName,root])=>[workspaceName,new Workspace(root)]));
      const select=(workspaceName?:string)=>{
      ...
      switch(name as N){
        ...
        case'list_directory':{
          const x=schemas.list_directory.parse(a);const {ws}=select(x.workspace);r=await ws.list(x.path,x.offset,x.limit);break;
        }
        case'workspace_tree':{
          const x=schemas.workspace_tree.parse(a);const {ws}=select(x.workspace);r=await ws.tree(x.path,x.maxDepth,x.maxEntries);break;
        }
        case'stat_path':{
          const x=schemas.stat_path.parse(a);const {ws}=select(x.workspace);r=await ws.stat(x.path);break;
        }
        case'find_files':{
          const x=schemas.find_files.parse(a);const {ws}=select(x.workspace);r=await ws.findFiles(x.path,x.pattern,x.maxResults);break;
        }
        case'search_files':{
          const x=schemas.search_files.parse(a);const {ws}=select(x.workspace);r=await ws.search(x.path,x.query,x.regex,x.caseSensitive,x.maxResults,x.contextLines);break;
        }
        case'read_file':{
          const x=schemas.read_file.parse(a);const {ws}=select(x.workspace);r={path:x.path,content:await ws.read(x.path)};break;
        }
        case'read_file_lines':{
          const x=schemas.read_file_lines.parse(a);const {ws}=select(x.workspace);r=await ws.readLines(x.path,x.startLine,x.endLine);break;
        }
        case'write_file':{
          const x=schemas.write_file.parse(a);const {ws}=select(x.workspace);r=await ws.write(x.path,x.content,x.overwrite);break;
        }
        case'edit_file':{
          const x=schemas.edit_file.parse(a);const {ws}=select(x.workspace);
          const old=await ws.read(x.path),i=old.indexOf(x.oldText);
          if(i<0||old.indexOf(x.oldText,i+1)>=0)throw new Error('oldText must match exactly once');
          r=await ws.write(x.path,old.slice(0,i)+x.newText+old.slice(i+x.oldText.length),true);
          break;
        }
        case'apply_patch':{
          const x=schemas.apply_patch.parse(a);const {ws}=select(x.workspace);r=await ws.applyEdits(x.path,x.edits,x.expectedSha256);break;
        }
        case'create_directory':{
          const x=schemas.create_directory.parse(a);const {ws}=select(x.workspace);r=await ws.createDirectory(x.path);break;
        }
        case'delete_path':{
          const x=schemas.delete_path.parse(a);const {ws}=select(x.workspace);r=await ws.delete(x.path,x.recursive);break;
        }
        case'move_path':{
          const x=schemas.move_path.parse(a);const {ws}=select(x.workspace);r=await ws.move(x.from,x.to,x.overwrite);break;
        }
        case'run_command':{
          const x=schemas.run_command.parse(a);const {ws}=select(x.workspace);r=await runCommand(x.command,await ws.path(x.cwd),x.timeoutMs);break;
        }
        case'start_process':{
          const x=schemas.start_process.parse(a);const {ws}=select(x.workspace);r=processes.start(x.command,await ws.path(x.cwd));break;
        }
        case'read_process':{
          const x=schemas.read_process.parse(a);r=processes.read(x.processId,x.stdoutCursor,x.stderrCursor);break;
        }
        case'write_process':{
          const x=schemas.write_process.parse(a);r=processes.write(x.processId,x.input);break;
        }
        case'stop_process':{
          const x=schemas.stop_process.parse(a);r=processes.stop(x.processId);break;
        }
        case'list_processes':
          r=processes.list();
          break;
